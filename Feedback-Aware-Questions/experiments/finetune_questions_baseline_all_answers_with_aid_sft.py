...
import copy
from prepare_dataset_questions import get_dataset_questions_all_answers_with_aid_sft
...
wandb.init(
    ...
    config={
        ...
        'task': 'Questions, All Answers With Aid, SFT',
    }
)

data_train, data_test, _ = get_dataset_questions_all_answers_with_aid_sft()


model_name = "meta-llama/Meta-Llama-3-8B"

new_model_name = "llama3_8b_questions_baseline_all_answers_with_aid_sft_lw"
...
data_train = [data for data in data_train if len(tokenizer(data['prompt'])['input_ids']) < 2000]
data_test = [data for data in data_test if len(tokenizer(data['prompt'])['input_ids']) < 2000]
print(len(data_train))
...
def _extract_text_between_braces(text):
    ...
    return words_indexes_between_braces
