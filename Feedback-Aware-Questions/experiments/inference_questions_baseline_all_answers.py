...
model_name = "ORGANIZATION_NAME/llama3_8b_questions_answer_selection_baseline_all_pairs_generation_v2"
...
def get_next_prompt(prompt, answer):
...
for data in tqdm(data_test):
    ...
    while iter < max_iter:
        ...
        try:
            ...
            prompt = get_next_prompt(prompt, answer)
        except Exception as e:
            print(e)
            break
    test_generated_list.append(gen_list)

json.dump(test_generated_list, open(f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_baseline_all_answers_sft_smart_duplicates.json", 'w'), indent=4)
