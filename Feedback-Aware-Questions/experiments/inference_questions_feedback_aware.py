...
model_name = "ORGANIZATION_NAME/llama3_8b_questions_feedback_aware_sft"
...
def get_next_prompt(prompt, answer, question, label):
    return f"{prompt}{label}" + " { " + answer + ' } ' + question + "\n"
...
for data in tqdm(data_test):
    ...
    while iter < max_iter:
        ...
        try:
            ...
            response = get_top_responses(responses, losses, [x['answer'] for x in gen_list])[0]
            answer = response
            ...
            prompt = get_next_prompt(prompt, answer, question, label)
        except Exception as e:
            print(e)
            break
    test_generated_list.append(gen_list)

json.dump(test_generated_list, open(f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_feedback_aware_sft_smart_duplicates.json", 'w'), indent=4)
