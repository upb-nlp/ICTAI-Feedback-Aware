...
def generate_question(texts, answers):
    model_inputs = tokenizer([f"Generate a question based on the context and the answer.\nContext: {text}\nAnswer: {answer}\n### Response:" for text, answer in zip(texts, answers)], return_tensors="pt", max_length=2048, truncation=True, padding='longest').to(device)
...
for data_test, data_name in zip([dataset_fairytaleqa, dataset_hotpot, dataset_narrativeqa, dataset_squad], ["fairytaleqa", "hotpotqa", "narrativeqa", "squad"]):
    ...
    for i in tqdm(range(0, len(data_test), batch_size)):
        ...
        answers = [data['answer'] for data in data_test[i:end_interval]]

        response = generate_question(texts, answers)
        responses += response

    for data, response in zip(data_test, responses):
        data['generated_question'] = response

    json.dump(data_test, open(f"Feedback-Aware-Questions/prerequisites_models/generated_questions_{data_name}_deterministic.json", "w"), indent=4)
